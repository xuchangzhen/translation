...
1. 将 `.env.example` 复制为 `.env`，填写已解析到服务器的域名和数据库随机长密码。
2. 确保服务器防火墙开放 80/443，域名 A/AAAA 记录已生效；共享 Nginx 服务器无需开放 18787。
3. 运行 `docker compose up -d --build`。
4. 打开 `https://你的域名/healthz`，应返回 `{"ok":true}`。

桌面端首次连接时只需填写 `https://你的域名`，随后会生成供安卓手机扫描的本地二维码，不再要求服务器注册码。每个设备创建时都会生成独立的随机读写令牌，内容密钥只保留在配对设备中。独占服务器模式由 Caddy 自动申请和续期证书；共享服务器模式沿用 Nginx 与 Certbot，并安装一个先验证配置再 reload 的续期钩子。
