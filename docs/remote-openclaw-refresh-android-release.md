...
GitHub Release `v0.9.0` 已发布，Android 版本为 `0.4.0`、`versionCode=5`。现有服务器镜像还可能停留在 `0.3.1`，请在服务器上执行以下提示词：
...
刷新后验证：
1. `curl --fail --silent https://memory.xuchangzhen968.top/android/latest.json` 的 packageName 是 com.linguabridge.memory，versionCode 是 5，versionName 是 0.4.0，sourceUrl 指向 GitHub v0.9.0，url 指向 `linguabridge-memory-0.4.0.apk`。
